...
public class CamCommandServer extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length == 0) {
			...
			sender.sendMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server interpolation <path> <" + String.join(":", CamInterpolation.getMovementNames()) + "> " + ChatFormatting.RED + "set the camera interpolation"));
			sender.sendMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server target <path> <none:self:pos:entity> " + ChatFormatting.RED + "set the camera target"));
			sender.sendMessage(new TextComponentString("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam-server add <path> <x> <y> <z> [yaw] [pitch] [roll] [zoom] " + ChatFormatting.RED + "adds a new point to path or creates a new one"));
