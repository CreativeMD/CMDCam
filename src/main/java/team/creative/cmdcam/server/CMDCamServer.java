...
public class CMDCamServer {
    ...
    private static final Function<CompoundTag, CamSaveData> deserializer = CamSaveData::new;

    public static CamScene get(Level level, String name) {
        CamSaveData data = ((ServerLevel) level).getDataStorage().get(deserializer, CamSaveData.DATA_NAME);
        if (data != null)
    ...
    public static void markDirty(Level level) {
        ((ServerLevel) level).getDataStorage().get(deserializer, CamSaveData.DATA_NAME).setDirty();
    }
